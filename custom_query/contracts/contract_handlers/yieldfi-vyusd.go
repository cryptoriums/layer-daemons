...
var _ ContractHandler = (*YieldFiVyusd)(nil)

type YieldFiVyusd struct{}

// FetchValue calculates yieldFi-vYUSD price in USD by:
// 1. Getting vYUSD/USDC exchange rate from contract (18 decimals)
// 2. Getting USDC/USD price from cache
// 3. Multiplying: (vYUSD/USDC rate) * (USDC/USD price) = vYUSD/USD price
// 4. Adjusting for decimals: 18 from exchange rate + market param decimals
func (r *YieldFiVyusd) FetchValue(
	ctx context.Context, reader *reader.Reader,
	priceCache *pricefeedservertypes.MarketToExchangePrices,
	maxDataAge time.Duration,
) (float64, error) {
	fetchedAt := time.Now()
	result, err := reader.ReadContract(ctx, YIELDFI_VYUSD_CONTRACT, "exchangeRateScaled() returns (uint256)", nil)
	if err != nil {
		return 0, fmt.Errorf("failed to call exchangeRateScaled: %w", err)
	}
	if err := checkDataAge(fetchedAt, maxDataAge); err != nil {
		return 0, fmt.Errorf("yieldfi-vyusd: %w", err)
	}

	vyusdExchangeRate := new(big.Int).SetBytes(result)
	...
	value := new(big.Int).Mul(vyusdExchangeRate, usdcPriceBig)
