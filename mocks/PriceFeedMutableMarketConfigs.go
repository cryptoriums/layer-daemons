...
func (_m *PricefeedMutableMarketConfigs) UpdateMarkets(marketParams []types.MarketParam) (map[uint32]error, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func([]types.MarketParam) (map[uint32]error, error)); ok {
		return rf(marketParams)
	}
	if rf, ok := ret.Get(0).(func([]types.MarketParam) map[uint32]error); ok {
		r0 = rf(marketParams)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func([]types.MarketParam) error); ok {
		r1 = rf(marketParams)
	} else {
		r1 = ret.Error(1)
	}
